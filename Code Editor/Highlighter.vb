...
Public Module CodeHighlighterModule
   ...
   Public Function CreateTemplate() As String
      Try
         ...
         If Not TemplatePath = Nothing Then
            ...
            Return OpenDocument(NoDialog:=True)
         End If
      Catch ExceptionO As Exception
         HandleError(ExceptionO)
      End Try
      ...
      Try
         For Index As Integer = 0 To SearchList.Count - 1
            If Code.StartsWith(SearchList(Index), CompareMethod) Then
               If IsKeyword Then
                  If Code.Length = SearchList(Index).Length OrElse (Not Char.IsLetterOrDigit(ToChar(Code.Substring(SearchList(Index).Length, 1)))) Then
                     Return Index
                  End If
               Else
                  Return Index
   ...
   'This procedure highlights the specified code with the specified color.
   Private Sub Highlight(Target As RichTextBox, Offset As Integer, Length As Integer, Color As Color)
      Try
         With Target
   ...
   Public Sub HighlightCode(Target As RichTextBox, Template As TemplateStr)
      Try
         Dim Buffer As New RichTextBox With {.Rtf = Target.Rtf, .Text = Target.Text}
               ...
               If CommentStart Is Nothing Then
                  ...
                  Else
                     If .Text.Substring(Position).StartsWith(Template.StringLiterals(StringLiteral.Value), CompareMethod) Then
                        ...
                        Highlight(Buffer, Offset, Math.Abs(Position - Offset) + 1, Settings.StringColor)
               ...
               If StringLiteral Is Nothing Then
                  ...
                  Else
                     If .Text.Substring(Position).StartsWith(Template.CommentEnd(CommentStart.Value), CompareMethod) Then
                        ...
                        Highlight(Buffer, Offset, Math.Abs(Position - Offset), Settings.CommentColor)
               ...
               If CommentStart Is Nothing AndAlso StringLiteral Is Nothing Then
                  ...
                  If Keyword IsNot Nothing Then
                     If PreviousCharacter = Nothing OrElse Not Char.IsLetterOrDigit(PreviousCharacter) Then
                        ...
                        Highlight(Buffer, Position, Fragment.Length, Settings.KeyWordColor)
                        Position += Fragment.Length
                     End If
                  End If
               End If

               If Position < .Text.Length Then PreviousCharacter = ToChar(.Text.Substring(Position, 1))
               Position += 1
            Loop
