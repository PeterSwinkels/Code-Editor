...
Public Module CoreModule
   Public Const DOCUMENT_PATH As Char = "%"c   'Defines the place holder for the path to the current document in an external program's arguments.
   ...
   Public DocumentO As New DocumentStr          'Defines the current document.
   Public HighlightingTemplate As TemplateStr   'Defines the current code highlighting template.
   Public Settings As New SettingsStr           'Defines the settings used by this program.
   ...
   Public Sub LaunchExternalProgram()
      Try
         ...
         ElseIf Settings.ExternalProgram = Nothing Then
            MessageBox.Show("A program to needs to be specified first.", My.Application.Info.Title, MessageBoxButtons.OK, MessageBoxIcon.Information)
         Else
            With New Process
               With .StartInfo
                  .Arguments = Settings.ExternalArguments
                  Position = .Arguments.IndexOf(DOCUMENT_PATH)
                  .Arguments = $"{ .Arguments.Substring(0, Position - 1)}{DocumentO.PathO}{ .Arguments.Substring(Position + 1)}"
   ...
   'This procedure opens a document.
   Public Function OpenDocument(Optional NoDialog As Boolean = False) As String
      Try
         ...
         Return File.ReadAllText(DocumentO.PathO)
      Catch ExceptionO As Exception
         HandleError(ExceptionO)
      End Try
