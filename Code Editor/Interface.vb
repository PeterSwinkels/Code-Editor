...
Public Class InterfaceWindow
   ...
   Private Sub ExternalArgumentsMenu_Click(sender As Object, e As EventArgs) Handles ExternalArgumentsMenu.Click
      Try
         ...
         Do
            NewArguments = ShowInputDialog($"Arguments ({DOCUMENT_PATH} = current document)", NewArguments, Button)
   ...
   Private Sub HighlightCodeMenu_Click(sender As Object, e As EventArgs) Handles HighlightCodeMenu.Click
      Try
         If HighlightingTemplate.Language = Nothing Then
            MessageBox.Show("A template must be loaded first.", My.Application.Info.Title, MessageBoxButtons.OK, MessageBoxIcon.Information)
         Else
   ...
   Private Sub InterfaceWindow_Load(sender As Object, e As EventArgs) Handles Me.Load
      Try
         Settings = If(File.Exists(Locations().SettingsPath), LoadSettings(Locations().SettingsPath), DefaultSettings())
         ...
         If Not DocumentO.PathO = Nothing Then
            TextBox.Text = OpenDocument(NoDialog:=True)
            UpdateTitleBar()
         End If
   ...
   'This procedure updates this window's title bar text.
   Private Sub UpdateTitleBar()
      Try
         With My.Application.Info
            Me.Text = $"{ProgramInformation()}{If(DocumentO.PathO = Nothing, Nothing, $" - {DocumentO.PathO}")}"
         End With
      Catch ExceptionO As Exception
         HandleError(ExceptionO)
      End Try
